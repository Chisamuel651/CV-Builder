...
const Share = () => {
    ...
    return (
        <div>
            <Popover>
                <PopoverTrigger disabled={resumeInfo?.status === 'archived' ? true : false} asChild>
                ...
                <PopoverContent className='bg-background' align='end' alignOffset={0} forceMount>
                    {resumeInfo?.status === 'public' ? (
                        <div className='space-y-3'>
                            <div className='flex gap-x-2 items-center'>
                                <Globe size='15px' className='text-primary animate-pulse' />
                                <p className='font-medium text-xs text-primary'>This resume is shareable, please copy the link!</p>
                            </div>

                            <div className="flex items-center">
                                <input className='border rounded-l-md bg-muted truncate flex-1 px-2 text-xs h-8' value={url} />
                                <Button
                                    className='h-8 rounded-l-none' disabled={copied}
                                    onClick={onCopy}
                        ...
                        <div className='w-full flex flex-col gap-2 items-center justify-center'>
                            ...
                            <div className="text-center mb-1">
                                ...
                                <p className='font-medium text-xs text-primary'>To share it with others, you need to make it public.</p>
                            </div>

                            <Button className='w-full h-8 !bg-black textx-xs dark:!bg-primary gap-1 font-semibold text-white' type='button' onClick={() => handleClick('public')}>
                                {isLoading && <Loader size='15px' className='animate-spin' />}
                                Public
                            </Button>
                        </div>
                    )}
